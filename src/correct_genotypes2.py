    ...
    def mendelProbsSingle(kid:int, back:int, elimination_order="MinNeighbors"):
        ...
        for j, SNP_id in enumerate(genotypes.columns):
            ...
            if len(parents) > 0:
                ...
                #if sum(np.isnan(anc_probs)) > 0: # this often happens when there are too many 9 in the immediate pedigree
                    ...
                    for kid in tqdm(corrected_genotype.index):
                        ...
                        if lddist != 'none':
                            for j, SNP_id in enumerate(corrected_genotype.columns):
                                ...
                                observedstatesevidence = {snpid:corrected_genotype.loc[kid,snpid] for snpid in empK.getWindow(SNP_id)}
                                if observed_state in [0,1,2]:
                                    empiricalcount = empK.getCountTable(observedstatesevidence,SNP_id)
                        ...
                        with tqdm(total=len(futures)) as pbar:
                            for future in concurrent.futures.as_completed(futures) :
                                ...
                                if resultPair is not None:
                                    ...
                                    emp_sire = cluster_emp[clusters_index[resultPair.sire]] #almost certainly the same 
                                    emp_dam = cluster_emp[clusters_index[resultPair.dam]] #almost certainly the same
                    ...
                    with tqdm(total=len(futures)) as pbar:
                        for future in concurrent.futures.as_completed(futures) :
                            ...
                            if resultkid is not None:
                                ...
                                emp_kid = cluster_emp[clusters_index[resultkid.kid]]
                                ...
                                for j,SNP_id in [(i,snp) for i, snp in enumerate(corrected_genotype.columns) if snp in resultkid.prob.keys()]:
                                    ...
                                    if lddist != 'none': # this overwrites the probabilities with product of empirical and mendelian 
                                        snpwindow = emp_kid.getWindow(SNP_id)
                                        observedstatesevidence = {snpid:corrected_genotype.loc[resultkid.kid,snpid] for snpid in snpwindow}
                                        state_obs = list(emp_kid.getCountTable(observedstatesevidence, SNP_id))
